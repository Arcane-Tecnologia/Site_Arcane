...
const siteConfig = await source('./src/lib/site-config.ts')
...
const conversionCtas = await source('./src/components/ui/conversion-ctas.tsx')
const servicesPage = await source('./src/app/(site)/services/page.tsx')
const projectsPage = await source('./src/app/(site)/projects/page.tsx')
...
assert.match(header, /ctas\.whatsapp\.href[\s\S]*Falar no WhatsApp/, 'header deve priorizar o WhatsApp como ação principal')
assert.match(header, /lg:flex/, 'navegação desktop deve começar em larguras grandes')
assert.match(header, /lg:hidden/, 'menu móvel deve permanecer ativo até a largura desktop')
assert.doesNotMatch(header, /md:flex|md:hidden/, 'header não deve quebrar na faixa tablet')
...
assert.doesNotMatch(conversionCtas, /linear-gradient\(135deg,#d29b7c_0%/, 'CTAs escuros não devem começar em bronze claro com texto branco')
assert.doesNotMatch(conversionCtas, /border-brand-cyan bg-brand-cyan text-white/, 'CTAs claros devem usar bronze forte com texto branco')
assert.doesNotMatch(servicesPage, /border-brand-cyan bg-brand-cyan text-white/, 'CTA de serviços deve respeitar contraste AA')
assert.doesNotMatch(projectsPage, /border-brand-cyan bg-brand-cyan text-white/, 'CTA de projetos deve respeitar contraste AA')
assert.doesNotMatch(referenceHome, /border-\[#c9ad96\] bg-\[#fffaf4\][\s\S]*text-\[#8e5c44\]/, 'bordas de CTAs claros devem ter contraste de componente')
assert.doesNotMatch(referenceHome, /pointer-events-none absolute inset-0 opacity-60/, 'grid decorativo da hero não deve interferir na análise de contraste')
assert.match(referenceHome, /function ReferenceHero[\s\S]*?initial=\{false\}/, 'conteúdo essencial da hero deve estar visível antes da animação')
